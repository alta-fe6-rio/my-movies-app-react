	...
	render() {
		return (
			<div className='container grow flex flex-col bg-slate-500 max-w-sm mx-auto rounded-md text-white'>
				<div onClick={this.props.onClickItem} className='cursor-pointer flex flex-col'>
					<img src={this.props.img ? `https://image.tmdb.org/t/p/w500${this.props.img}` : 'https://via.placeholder.com/500x750?text=No+Image'} alt={this.props.img} width='350' height='450' className='mx-auto' />
					<Link to='/movie/3' className='text-center text-2xl font-bold p-5'>
						{this.props.title}
					</Link>
				</div>
